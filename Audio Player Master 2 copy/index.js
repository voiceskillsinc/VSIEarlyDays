...
const CARDTITLE = 'Cruise Ship Tilt';
const CARDTEXT = 'Ten Minutes of Terror';
const SMALLIMAGE = 'https://s3.amazonaws.com/shortstoryassetsvsi/tenMinutesofTerror/cships.png';
const LARGEIMAGE = 'https://s3.amazonaws.com/shortstoryassetsvsi/tenMinutesofTerror/cshipL.png';
const BACKGROUND_IMAGE_URL = 'https://s3.amazonaws.com/shortstoryassetsvsi/tenMinutesofTerror/cshipL.png';
///////////////////////////////
...
const LaunchRequestHandler = {
  ...
  async handle(handlerInput) {
    ...
    if (!playbackInfo.hasPreviousPlaybackSession) {
      message = 'Welcome to Cruise Ship Tilt. Would you like to hear the story?';
      reprompt = 'Just say play, to begin.';
    } else {
      playbackInfo.inPlaybackSession = false;
      message = `You were listening to ${constants.audioData[playbackInfo.playOrder[playbackInfo.index]].title}. Would you like to continue?`;
      reprompt = 'Say yes to resume or no to play from the begining.';
    }
    
    if (supportsDisplay(handlerInput)) {
          
          const BACKGROUND_IMAGE_URL = 'https://s3.amazonaws.com/shortstoryassetsvsi/tenMinutesofTerror/cshipL.png';
          const BACKGROUND_IMAGE = 'https://s3.amazonaws.com/shortstoryassetsvsi/tenMinutesofTerror/cships.png';
          const CARDTEXT = "";
          const TITLE = 'Cruise Ship Tilt';
          const TEXT = '';
          ...
          let smallImage = new alexa.ImageHelper()
              .withDescription(TITLE)
              .addImageInstance(BACKGROUND_IMAGE)
              .getImage();
        ...
        handlerInput.responseBuilder
            ...
            .withStandardCard(TITLE, CARDTEXT, LARGEIMAGE, SMALLIMAGE);
    }

    return handlerInput.responseBuilder
      .speak(message)
      .reprompt(reprompt)
      .withStandardCard(CARDTITLE, CARDTEXT, LARGEIMAGE, SMALLIMAGE)
      .getResponse();
  }
};
      ...
      case 'PlaybackNearlyFinished':
        {
          ...
          const story = constants.audioData[playbackInfo.playOrder[enqueueIndex]];
...
const CheckAudioInterfaceHandler = {
  ...
  handle(handlerInput) {
    return handlerInput.responseBuilder
      .speak('Sorry, this Cruise Ship Tilt is not supported on this device')
...
const StartPlaybackHandler = {
  async canHandle(handlerInput) {
    ...
    if (!playbackInfo.inPlaybackSession) {
      return request.type === 'IntentRequest' && request.intent.name === 'PlayStreamIntent';
    }
...
const HelpHandler = {
  ...
  async handle(handlerInput) {
    const playbackInfo = await getPlaybackInfo(handlerInput);
    let message;
    const cardTitle = 'What can I help you with?';

    if (!playbackInfo.hasPreviousPlaybackSession) {
      message = 'Welcome to Cruise Ship Tilt. Would you like to hear the story?';
    } else if (!playbackInfo.inPlaybackSession) {
      message = `You were listening to ${constants.audioData[playbackInfo.index].title}. Would you like to continue?`;
    } else {
    ...
    return handlerInput.responseBuilder
      .speak(message)
      .reprompt(message)
      .withStandardCard(cardTitle, message, LARGEIMAGE, SMALLIMAGE)
      .getResponse();
  },
};
...
const ExitHandler = {
  ...
  handle(handlerInput) {
    const cardTitle = 'Cruise Ship Tilt';
    const cardText = "Thanks for listening to the Ten Minutes of Terror Story series please leave us a review.";
    
    return handlerInput.responseBuilder
      .speak('Goodbye, thanks for listening to the Ten Minutes of Terror Story series please leave us a review.')
      .withStandardCard(cardTitle, cardText, LARGEIMAGE, SMALLIMAGE)
      .getResponse();
  },
};
...
const ErrorHandler = {
  ...
  handle(handlerInput, error) {
    ...
    const textTitle = 'Error in the Skill!';
    const message = (`Error handled: ${error.message}`);
    const errorMessage = 'There was an error in the Skill!';

    return handlerInput.responseBuilder
      .speak(errorMessage)
      .reprompt(errorMessage)
      .withStandardCard(textTitle, message, LARGEIMAGE, SMALLIMAGE)
      .getResponse();
  },
};
//////////////////////////
...
const controller = {
  async play(handlerInput) {
    ...
    const story = constants.audioData[playOrder[index]];
    const token = playOrder[index];

    responseBuilder
      .speak(`This is ${story.title}`)
      .withShouldEndSession(true)
      .addAudioPlayerPlayDirective(playBehavior, story.url, token, offsetInMilliseconds, null, story.metadata);

    if (await canThrowCard(handlerInput)) {
      const cardTitle = `Playing ${story.title}`;
      const cardContent = `Playing ${story.title}`;
      responseBuilder.withStandardCard(cardTitle, cardContent, LARGEIMAGE, SMALLIMAGE);
    }
